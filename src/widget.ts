...
export class TurtleView extends DOMWidgetView {
  ...
  update() {
    const commands = (this.model.get('commands') || []).filter(
      (c: any) => c.id > this.lastCommandId
    );
    if (!commands.length) {
      return;
    }
    ...
    commands.forEach((command: any) => {
      ...
      if (type === 'line') {
        ...
        const { x, y, radius, color, extent, lineWidth, start } = command;
        this.drawCircle(x, y, radius, color, extent, lineWidth, start);
      }
